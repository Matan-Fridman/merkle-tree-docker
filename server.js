...
const app = express();
const fs = require("fs")
const {addCommitment, generateAndVerifyProof, getFileNames} = require("./zk-functions")
...
app.post("/add_commitments_from_bucket", async (req, res)=>{ // trigger request from bucket to add commitments
    ...
    await addCommitment(commitmentsToAdd)
    res.send(commitmentsToAdd)
})
app.post("/gen_proof", async (req, res)=>{
    // const commitment = await addCommitment()
    const commitment = req.body
    const commitments = (await fs.promises.readFile("google-cloud-downloads/merkle-tree-commitments", "utf-8")).split(", ")
    const verif = await generateAndVerifyProof(commitments, commitment)
    console.log(verif)
    res.send(verif)
})
...
// export async function generateAndVerifyProof(commitments, commitment){
//     const mimc = await buildMimcSponge()
